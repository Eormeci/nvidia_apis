...
def gradio_interface():
    with gr.Blocks() as demo:
        ...
        # Radio button ile normal çıktı veya JSON formatı seçimi
        output_format = gr.Radio(choices=["Normal Çıktı", "JSON Formatında"], label="Çıktı Formatı", value="Normal Çıktı")

        # Gönderme butonu
        submit_button = gr.Button("API'ya Gönder ve Yorumla")

        # Videodan anlık kareyi alıp API'ye gönderme
        def process_frame_from_video(video, prompt, model, output_format_choice):
            ...
            if image:
                ...
                # Kullanıcı JSON formatı istiyorsa, çıktıyı JSON formatına dönüştür
                if output_format_choice == "JSON Formatında":
                    result_json = create_openai_compatible_json(result)
                    return json.dumps(result_json, indent=2), image  # JSON formatında döndür
                else:
                    return result, image  # Normal çıktı döndür
            else:
                return "Anlık kare alınamadı.", None

        # Butona tıklandığında işlemi başlat
        submit_button.click(process_frame_from_video, inputs=[video_input, prompt_input, model_choice, output_format], outputs=[output_text, output_image])
